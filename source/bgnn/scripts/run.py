...
class RunModel:
    ...
    def get_input(self, dataset_dir, dataset: str):
        ...
        elif dataset in DATASET_NAMES:
            input_folder = dataset_dir / dataset
        else:
            input_folder = dataset

        if self.save_folder is None:
            self.save_folder = f'results/{dataset}'
    ...
    def run_one_model(self, config_fn, model_name):
        ...
        for ps in grid:
            ...
            self.store_results[exp_name] = (
                list(map(np.mean, zip(*runs))),

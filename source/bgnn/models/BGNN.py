...
class BGNN(BaseModel):
    ...
    def train_gbdt(self, gbdt_X_train, gbdt_y_train, cat_features, epoch,
        ...
        if epoch == 0 and self.task in ['multiclass', 'binclass']:
    ...
    def fit(self, edgelist, X, y, train_mask, val_mask, test_mask, cat_features,
        ...
        if metric_name in ['r2', 'accuracy', 'roc-auc']:
        ...
        self.y = y
        if self.lang == 'dgl':
            graph = self.edgelist_to_torch(edgelist)
        elif self.lang == 'pyg':
            graph = None
        ...
        for epoch in pbar:
            ...
            best_metric, best_val_epoch, epochs_since_last_best_metric = \
                self.update_early_stopping(metrics, epoch, best_metric, best_val_epoch, epochs_since_last_best_metric,
                                           metric_name, lower_better=(metric_name not in ['r2', 'accuracy', 'roc-auc']))
